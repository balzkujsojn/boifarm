                ...
                if not isPlayerCharacter then
                    ...
                    if targetPart then
                        local enemyData = {
                            ...
                            Name = characterName,
                            IsPriority = PRIORITY_ENEMIES[characterName] == true,
                            LastSeen = now,
...
local function selectTarget()
    ...
    -- Check if we are locked onto Arbiter
...
local function attemptFire()
    ...
    local targetPos
    if isArbiter then
        -- FIXED: Priority to actual Part position, fallback to hardcoded only if necessary
        if targetResult and targetResult.TargetPart then
    ...
    if not fireSuccess then
        ...
        -- Retry logic for Arbiter with fail-safe position
        if isArbiter then
            task.wait(0.1)
            pcall(function()
                -- Try firing directly at position
